    ...
    ui(new Ui::ChannelH22)
{
    ...
    addrSrv_chl4.sin_port=htons(7004);//server : receive port number
...
void ChannelH22::rendermap(char *buff){
   ...
   //divide the data into 4 road
    for( int i = 0 ; i < 1800 ; i++){
        ...
        data2_4[i][0]=hex2int(map1800_4[i][8],map1800_4[i][9],map1800_4[i][10],map1800_4[i][11]);
        data2_4[i][1]=hex2int(map1800_4[i][12],map1800_4[i][13],map1800_4[i][14],map1800_4[i][15]);
...
*/
    ...
    double pilotH22[100][2] = {0};
    int cntH22 = 0;
    for( int i = 0 ; i < 300 ; i++){
        if(i % 6 == 3){//position for the pilot H22
            pilotH22[cntH22][0] = data2_4[i][0];
            pilotH22[cntH22][1] = data2_4[i][1];
            cntH22 += 2;
        }//if
    }
    ...
    for( int i = 1200 ; i < 1500 ; i++){
        if( i % 6 == 0){//position for the pilot H22
            pilotH22[cntH22][0] = data2_4[i][0];
            pilotH22[cntH22][1] = data2_4[i][1];
            cntH22 += 2;
        }//if
    ...
    if(first1 == 1){
         first1 = 0;
         FILE *fp3 = fopen("./pilot/pilotH22.txt","w");

        for( int i = 0 ; i <100 ; i++){
            fprintf(fp3,"%.0lf\t\t%.0lf\n",pilotH22[i][0],pilotH22[i][1]);
        }
        fclose(fp3);
     }
    */
    double H22[100][2] = {0};
    double absH22[100] = {0};
    double absH22_3[300] = {0};

    for( int i = 0 ; i < 100 ; i++ ){
        H22[i][0] = (pilotH22[i][0]*pilot4[i][0]+pilotH22[i][1]*pilot4[i][1])/(pilot4[i][0]*pilot4[i][0]+pilot4[i][1]*pilot4[i][1]);
        H22[i][1] = (pilotH22[i][1]*pilot4[i][0]-pilotH22[i][0]*pilot4[i][1])/(pilot4[i][0]*pilot4[i][0]+pilot4[i][1]*pilot4[i][1]);
        absH22[i] = sqrt(H22[i][0]*H22[i][0] + H22[i][1]*H22[i][1]);
        absH22_3[i*3] = absH22[i];
         absH22_3[i*3 + 1 ] = absH22[i];
          absH22_3[i*3 + 2 ] = absH22[i];
    }//for i
/*
    if(first1 == 1){
         first1 = 0;
         FILE *fp3 = fopen("./pilot/H22.txt","w");

        for( int i = 0 ; i <300 ; i++){
            fprintf(fp3,"%.3lf\t\t%.3lf\t\t\t%.3lf\n",absH22_3[i],absH22_3[i],absH22_3[i]);
        }
        fclose(fp3);
     }
     */
